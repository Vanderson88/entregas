...
import 'package:entregas/src/pages/restaurant/orders/detail/restaurant_orders_detail_controller.dart';
import 'package:entregas/src/utils/my_colors.dart';
import 'package:entregas/src/utils/relative_time_until.dart';
import 'package:entregas/src/widgets/no_data_widget.dart';
...
class _RestaurantOrdersDetailPageState extends State<RestaurantOrdersDetailPage> {
    ...
    return Scaffold(
          ...
          child: Column(
            children: [
              Divider(
                ...
                endIndent: 30, // DIREITA
                indent: 30, //ESQUERDA
              ),
              SizedBox(height: 10),
              _textDescription(),
              SizedBox(height: 15),
              _con.order.status != 'PAGO' ? _deliveryData() : Container(),
              _con.order.status == 'PAGO' ? _dropDown(_con.users) : Container(),
              _textData('Cliente:', '${_con.order.client?.name ?? ''} ${_con.order.client?.lastname ?? ''}'),
              _textData('Entregar em:', '${_con.order.address?.address ?? ''}'),
              _textData(
                  'Data do pedido:',
                  '${RelativeTimeUtil.getRelativeTime(_con.order.timestamp ?? 0)}'
              ),
              _con.order.status == 'PAGO' ? _buttonNext() : Container()
            ],
          ),
        ),
      ),
      body: _con.order.products.length > 0
        ...
        children: _con.order.products.map((Product product) {
          ...
          : NoDataWidget(text: 'Nenhum producto adicionado',),
    );
  }
  ...
  Widget _textDescription() {
    return Container(
      ...
      child: Text(
        _con.order == 'PAGO' ? 'Atribuir entregador' : 'Entregador atribuido',
        style: TextStyle(
            ...
            children: [
              Container(
                ...
                child: DropdownButton(
                  ...
                  onChanged: (option) {
                    setState(() {
                      print('Entregador selecciondo $option');
                      _con.idDelivery = option; // ESTABELECENDO O VALOR SELECCIONADO
                    });
                  },
                ),
              )
