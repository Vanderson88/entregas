...
import 'package:entregas/src/pages/client/orders/detail/client_orders_detail_controller.dart';
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              SizedBox(height: 10),
              _textData('Entregador:', '${_con.order?.delivery?.name ?? 'Nao atribuido'} ${_con.order?.delivery?.lastname ?? ''}'),
              _textData('Entregar em:', '${_con.order?.address?.address ?? ''}'),
              _textData(
                  'Data do pedido:',
                  '${RelativeTimeUtil.getRelativeTime(_con.order.timestamp ?? 0)}'
              ),
              _con.order?.status == 'A CAMINHO' ? _buttonNext() : Container()
            ],
          ),
        ),
      ),
