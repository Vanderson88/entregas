...
class RegisterController {
  ...
  void register() async {
    ...
    if (confirmPassword != password) {
      MySnackbar.show(context, 'As palavras passes sao diferentes');
      return;
    }

    if (password.length < 6) {
      MySnackbar.show(context, 'As palavras passes devem ter ao menos 6 caracteres');
      return;
    }

    if (imageFile == null) {
      MySnackbar.show(context, 'Selecciona uma imagem');
      return;
    }
  ...
  void showAlertDialog() {
    ...
    AlertDialog alertDialog = AlertDialog(
      title: Text('Selecciona a tua imagem'),
      actions: [
        galleryButton,
        cameraButton
      ],
